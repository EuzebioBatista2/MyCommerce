...
export default function Input(props: IInputProps) {
  ...
  function verifyBlur(event: any) {
    if(event.target.value === '' && event.target.id === props.id) {
      event.target.value = ''
      setAnimation(false)
    } 
  }
  ...
  return (
    <div className="flex items-center justify-center h-14 relative px-1">
      ...
      <input type={props.type} name={props.id} id={props.id}
        className={`h-8 px-1 self-end border-b bg-gray-100 ${animation ? 'border-blue-500' : 'border-black'} outline-none w-full transtion duration-500 ease-in-out`}
